...
interface ArticleEditProps extends ConnectProps<{ id: string }> {}
...
const ArticleEdit: React.FC<ArticleEditProps> = (props) => {
  ...
  // 获取文章详情
  const { loading } = useRequest<ResponseResultType<IArticle>>(
    () => services.queryArticle(id, { include: 'tags' }),
    {
      onSuccess(article) {
        setPreviewUrl(article.preview);
        form.setFieldsValue({
          title: article?.title,
          state: article?.state,
  ...
  // 更新文章
  const { loading: submitting, run: updateArticle } = useRequest(services.updateArticle, {
    manual: true,
    onSuccess() {
      ...
      <Card bordered={false}>
        ...
        >
          <Form.Item name="title" label="标题" rules={[{ required: true, message: '请输入标题' }]}>
            <Input placeholder="给文章起个名字" disabled={loading} />
          </Form.Item>
