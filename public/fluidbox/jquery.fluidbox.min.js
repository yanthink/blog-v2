  ...
  a.extend(e.prototype, {
    init: function() {
      ...
      if (
        ...
        (!b.instanceData || !b.instanceData.initialized) &&
          c.is('a') &&
          1 === c.children().length &&
          (c.children().is('img') || (c.children().is('picture') && 1 === c.find('img').length)) &&
          'none' !== c.css('display') &&
    ...
    open: function() {
      ...
      if (
        ...
        return b.close(), !1;
      e.css({ 'background-image': 'url(' + m.formatURL(d.attr('src')) + ')', opacity: 1 }),
      ...
      b.settings.immediateOpen
        ? (c
            ...
            .find('.fluidbox__wrap')
            .css({ zIndex: b.settings.stackIndex + b.settings.stackIndexDelta }),
          ...
          (h.onload = function() {
            if ((c.trigger('imageloaddone.fluidbox'), 1 === b.instanceData.state)) {
              ...
              e.css({ 'background-image': 'url(' + m.formatURL(h.src) + ')' }), b.compute();
            }
          }),
          (h.onerror = function() {
            b.close({ error: !0 }),
        ...
        : ((h = new Image()),
          (h.onload = function() {
            return (
              ...
              m.checkURL(h.src)
                ? (b.close({ error: !0 }), !1)
                : (e.css({ 'background-image': 'url(' + m.formatURL(h.src) + ')' }),
                  (b.instanceData.thumb.natW = h.naturalWidth),
                  (b.instanceData.thumb.natH = h.naturalHeight),
                  b.compute(),
                  d.css({ opacity: 0 }),
    ...
    compute: function() {
      ...
      b.settings.maxWidth > 0
        ? ((h = b.settings.maxWidth), (j = h / m))
        : b.settings.maxHeight > 0 && ((j = b.settings.maxHeight), (h = j * m));
      var o, p, q, r, s;
      n > m
        ? ((o = j < i.viewport.h ? j : i.viewport.h * b.settings.viewportFill),
          (q = o / l),
          (r = (h * ((l * q) / j)) / k),
          (s = q))
        : ((p = h < i.viewport.w ? h : i.viewport.w * b.settings.viewportFill),
          (r = p / k),
          (q = (j * ((k * r) / h)) / l),
          (s = r)),
        b.settings.maxWidth &&
      ...
      var t = f.scrollTop() - d.offset().top + 0.5 * (l * (s - 1)) + 0.5 * (f.height() - l * s),
        ...
        : ((c.instanceData.state = 0),
          ...
          g.one(l, function() {
            g.css({ opacity: 0 }),
              f.css({ opacity: 1 }),
              i.remove(),
              h.css({ zIndex: c.settings.stackIndex - c.settings.stackIndexDelta }),
    ...
    bindListeners: function() {
      ...
      a.isFunction(a.throttle)
        ? f.on('resize.fluidbox' + b.instanceData.id, a.throttle(b.settings.resizeThrottle, d))
